...
from src.modeling.custom_regularization_custom_loss import regularization_loss
...
def main(config_file):
    ...
    for epoch in tqdm(range(args_config["training_args"]["epochs"])):
        ...
        for step, train_batch in enumerate(train_loader):
            ...
            if args_config["model"]['type'] == "custom_regularization":
                outputs, e3 = model(train_batch, b_t=b_t)
                loss = nn.MSELoss()(outputs, train_batch) + 0.01 * regularization_loss(e3)
            else:
