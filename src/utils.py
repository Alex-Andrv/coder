...
def process_images(test_loader, model, device, b, w=128, h=128):
    ...
    quantized_imgs_encoded = (torch.round(torch.clip(norm_imgs_encoded, 0, 0.9999999) * pow(2, b))).to(
        torch.int32
    )
    # print("kek")
    # pi_value = 3.141592653589793
    # e3 = torch.clip(norm_imgs_encoded, 0, 0.9999999) * pow(2, b)
    # loss = torch.mean(((torch.sin(e3 * 2.0 * pi_value - (pi_value / 2.0)) + 1) / 2) ** 2)
    # print(loss)
    # print("kek")
    ...
    print((quantized_imgs_decoded - quantized_imgs_encoded))
    shift = 1.0 / pow(2, b + 1)
    dequantized_imgs_decoded = (quantized_imgs_decoded.to(torch.float32) / pow(2, b)) 
...
def JPEGRDSingleImage(torch_img, TargetBPP):
    ...
    for Q in range(501):
        ...
        if (abs(realbpp - TargetBPP) > abs(bpp - TargetBPP)) or (final_image is None):
            realbpp = bpp
            realpsnr = psnr
            realQ = Q
